...
describe('classic engine', () => {
  ...
  it('gravity accelerates as level increases', () => {
    ...
    assert.ok(lateDrops > earlyDrops, `expected more drops at high level: ${lateDrops} vs ${earlyDrops}`)
  })
})
...
describe('classic score comparison', () => {
  it('ranks higher level first, then longer time', async () => {
    ...
    assert.ok(beatsClassicScore({ level: 12, duration_ms: 1000 }, { level: 10, duration_ms: 999_999 }))
    assert.ok(beatsClassicScore({ level: 10, duration_ms: 2000 }, { level: 10, duration_ms: 1000 }))
