    ...
    async action(context) {
      ...
      if (!parsed.success) {
        return context.render(
          <AuthPage
            mode="signup"
            error="Username must be 3-20 characters and password at least 6."
      ...
      let result = await createUser(db, username, password)
      if (!result.ok) {
        return context.render(
          <AuthPage mode="signup" error="That username is taken." username={username} returnTo={returnTo} theme={theme} />,
          { status: 400 },
        )
      }
