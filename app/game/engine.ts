...
import { cellsFor, kicksFor, PIECE_COUNT, type PieceType, type Rotation } from "./pieces.ts";
...
export type Action = "left" | "right" | "rotateCW" | "rotateCCW" | "softDrop" | "hardDrop" | "hold";
...
export function step(state: GameState, actions: readonly Action[] = []): GameState {
  ...
  for (let action of actions) {
    ...
    if (
      progressed &&
      !lockedByHardDrop &&
      (action === "left" || action === "right" || action === "rotateCW" || action === "rotateCCW")
    ) {
...
export function maxTicksForMode(modeId: string, recorded: readonly RecordedAction[] = []): number {
  let mode = getMode(modeId);
