...
export async function versusLeaderboard(
  ...
  for (let game of games) {
    if (game.winner_user_id != null) {
      ...
      if (!t) {
        t = { user_id: game.winner_user_id, wins: 0, games: 0, latestWinId: null, latestGameId: game.id }
    ...
    if (game.loser_user_id != null) {
      ...
      if (!t) {
        t = { user_id: game.loser_user_id, wins: 0, games: 0, latestWinId: null, latestGameId: game.id }
  ...
  let userRows = await db.findMany(users, { where: inList('id', ranked.map((t) => t.user_id)) })
